...
def max_session_uncertainty_us(session: dict[str, Any]) -> float:
    """Conservative cap: worst PASS-segment uncertainty in the session."""
    values: list[float] = []
    ...
    if not values:
        raise ValueError("Clock session has no PASS segment uncertainty_us")
    return max(values)


def build_parser() -> argparse.ArgumentParser:
    """Build the held-out NCCL validation CLI."""
    parser = argparse.ArgumentParser(
        description=(
            "Check aligned rank Traces for NCCL happened-before inversions. "
            "Does not rewrite files and does not feed the clock model."
        )
    )
    add_rank_trace_argument(
        parser,
        help_text=(
            "Aligned Trace as rank:path, repeatable. "
            "Example: 0:rank0.aligned.json"
        ),
    )
    parser.add_argument("--output", required=True, type=Path)
    uncertainty = parser.add_mutually_exclusive_group(required=True)
    uncertainty.add_argument("--uncertainty-us", type=float)
    uncertainty.add_argument("--clock-session", type=Path)
    return parser


def main() -> None:
    """Run the NCCL check and write a JSON report."""
    args = build_parser().parse_args()
    traces = dict(parse_rank_trace_arg(item) for item in args.traces)
    if len(traces) != len(args.traces):
        raise SystemExit("Duplicate rank in --trace arguments")
    try:
        if args.clock_session is not None:
            session = json.loads(args.clock_session.read_text(encoding="utf-8"))
            uncertainty_us = max_session_uncertainty_us(session)
        else:
            uncertainty_us = float(args.uncertainty_us)
        ...
        print(f"NCCL check failed: {error}", file=sys.stderr)
