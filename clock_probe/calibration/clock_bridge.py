...
def _fit_bridge(samples: Sequence[dict[str, float]]) -> dict[str, float]:
    ...
    base_ns, offset_at_base_ns, drift = fit_affine(
...
# Adjacent piecewise fits predict REALTIME independently, so a shared
# monotonic midpoint can leave a few-microsecond REALTIME hole. Fill those
# interior holes; do not extend past the first or last sampled bound.
_MAX_STITCH_GAP_NS = 50_000
...
class CompiledClockBridge:  # pylint: disable=too-few-public-methods
    ...
    def _select_segment(self, realtime_ns: int) -> dict[str, Any]:
        """Return the PASS segment covering ``realtime_ns``, including stitch gaps."""
        covering = [
            segment
        ...
        ordered.sort(key=lambda segment: int(segment["valid_from_realtime_ns"]))
        for previous, following in zip(ordered, ordered[1:]):
            prev_to = int(previous["valid_to_realtime_ns"])
            next_from = int(following["valid_from_realtime_ns"])
            if not prev_to < realtime_ns < next_from:
                continue
            if next_from - prev_to > _MAX_STITCH_GAP_NS:
                continue
            if realtime_ns - prev_to <= next_from - realtime_ns:
                return previous
            return following
        raise ValueError(
    ...
    def realtime_to_monotonic_ns(
        ...
        *,
        ...
        segment = self._select_segment(realtime_ns)
        ...
        lo = int(segment["valid_from_monotonic_ns"])
        hi = int(segment["valid_to_monotonic_ns"])
        if lo <= rounded_ns <= hi:
            return rounded_ns, float(segment["uncertainty_us"])
        if rounded_ns < lo and lo - rounded_ns <= _MAX_STITCH_GAP_NS:
            return lo, float(segment["uncertainty_us"])
        if rounded_ns > hi and rounded_ns - hi <= _MAX_STITCH_GAP_NS:
            return hi, float(segment["uncertainty_us"])
        raise ValueError(
